...
const COLUMNS = [
  ...
  { key: "agreementTotal", label: "Agreement", numeric: true },
...
export default function LeaderboardManager() {
  ...
  const compositeRows = useMemo(() => {
    ...
    const enriched = rows.map((r) => ({
      ...r,
      composite:
        weights.agreement * r.agreementTotal +
  ...
  const histogram = useMemo(() => {
    ...
    if (max <= 0) {
      return [{ bin: "0", reviewers: scores.length, lo: 0, hi: 0 }];
    }
    const binCount = 10;
  ...
  return (
    <div className="space-y-6">
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-900">
        ...
        <p>
          For each reviewer submission we compute three quality signals
          (agreement, length, missing-finding consensus), each in [0, 1].
          A time-penalty multiplier degrades the contribution of submissions
          that landed suspiciously fast. Per reviewer, we sum
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <CriterionCard
          title="1. Agreement — peer consensus on findings"
          fixed
        >
          <p>
            For every finding the reviewer judged, we compare their
            genuine/spurious verdict against every <em>other</em> submission on
            the same requirement (leave-one-out). Per-finding score is the
            fraction of others who agree; the submission&apos;s agreement is
            the mean across its findings.
          </p>
          <p className="mt-2 text-xs text-gray-500">
            First submitter on a requirement (no peers to compare against) is
        ...
        <CriterionCard title="2. Length — explanation effort">
          <p>
            ...
            (<em>explanation</em> on findings marked not genuine, plus
            <em> missingAmbiguities</em> / <em>missingClarifications</em> notes) by
            word count. Below <strong>minWords</strong> → 0; from minWords up
            to <strong>maxWords</strong> the score ramps linearly to 1.
            Submissions with no text fields (all findings accepted, no
        ...
        <CriterionCard
          title="3. Missing-finding consensus"
          fixed
        >
          <p>
            Treats each side (ambiguities / clarifications) as a binary: did this
            reviewer flag something missing? Per side we score against peers
            on the same requirement:
          </p>
          <ul className="mt-2 list-disc list-inside space-y-0.5 text-xs">
            <li>Reviewer flagged, 2+ others flagged → 1.0</li>
            <li>Reviewer flagged, exactly 1 other flagged → 0.75</li>
            <li>Reviewer flagged, no others flagged → 0</li>
            <li>Reviewer did not flag, no others flagged → 1.0</li>
            <li>Reviewer did not flag, majority of others flagged → 0</li>
            <li>Reviewer did not flag, minority of others flagged → 0.5</li>
          </ul>
          <p className="mt-2 text-xs text-gray-500">
            First submitter on a requirement: 1.0 per side.
          </p>
        </CriterionCard>

        <CriterionCard title="4. Time penalty (asymmetric)">
          <p>
            Per reviewer we sort submissions by <em>submittedAt</em> and use
            ...
            wall-clock idle time isn&apos;t engagement. The reviewer&apos;s very first
        ...
        <>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <WeightSlider
                ...
                onChange={(v) => setWeights(rescaleWeights(weights, "agreement", v))}
              />
              <WeightSlider
                label="Length"
          ...
          {histogram && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <p className="text-xs text-gray-500 mb-4">
                Reviewers binned by current composite score. Reacts to the
              ...
              <ResponsiveContainer width="100%" height={280}>
                ...
                >
                  ...
                  />
                  <Tooltip
                    formatter={(v) => [v, "Reviewers"]}
                    labelFormatter={(label) => `Score ${label}`}
                  />
                  <Bar dataKey="reviewers" fill="#3b82f6" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
